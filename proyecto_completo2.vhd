...
entity cont30 is
    port(
        ...
        reset  : in  std_logic;
        start  : in  std_logic;  -- señal de inicio (compra confirmada)
        done   : out std_logic   -- alta cuando termina la entrega (5 segundos)
    );
end cont30;

architecture arch_cont30 of cont30 is
    signal count   : unsigned(2 downto 0) := (others => '0'); -- cuenta hasta 5 (3 bits)
    signal active  : std_logic := '0';
begin
    process(clk, reset)
    begin
        if reset = '1' then
            ...
            if start = '1' and active = '0' then
                ...
                if count < 4 then  -- 0 a 4 = 5 segundos
                    count <= count + 1;
                else
                    active <= '0'; -- terminar después de 5s
                end if;
            end if;
        end if;
    end process;

    done <= not active;  -- Alta cuando no activa (terminada)
end arch_cont30;
library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;
...
entity fsm_maquina_expendedora is
    port(
        ...
        saldo       : in  integer range 0 to 9500;       -- Saldo actual (de RAM saldo)
        cambio      : in  integer range -9999 to 9999;   -- Cambio calculado
        clk_2s      : in  std_logic;                     -- Reloj para alerta (de div_2seg)
        entrega_done: in  std_logic;                     -- De cont30: entrega terminada
        ...
        ram_saldo_we: out std_logic;                     -- Write enable para RAM saldo
        ram_saldo_addr: out std_logic_vector(0 downto 0); -- Dirección para RAM saldo (0)
        ram_saldo_din: out integer range 0 to 9500;     -- Dato a escribir en RAM saldo
        ...
        valor_display_full : out integer range 0 to 9999; -- Cambio completo para disp0-disp3
        ...
        buzzer      : out std_logic;                     -- Señal sonora (alta en anomalía o entrega, o debug pulso)
        debug_venta : out std_logic;                     -- Debug: alta cuando venta completada (reset saldo)
        start_entrega : out std_logic                    -- Nueva: iniciar cont30 solo en DISPENSING
    );
...
architecture behavioral of fsm_maquina_expendedora is
    ...
    signal state, next_state : state_type := WAITING_SELECTION;
    
    signal producto_sel : integer range 0 to 15 := 0;
    signal stock_actual : integer range 0 to 3 := 0;
    signal stock_display : integer range 0 to 3 := 0;  -- Stock para LEDs (actualizado inmediatamente)
    ...
    signal confirm_latched : std_logic := '0';  -- Latched para confirmar compra
    signal venta_completada : std_logic := '0';  -- Para reset saldo y decremento
    signal buzzer_debug_cnt : integer range 0 to 50000000 := 0;  -- Contador para buzzer debug (~1s @50MHz)
    signal buzzer_debug : std_logic := '0';
    signal prev_coin500 : std_logic := '0';
    signal prev_coin1000 : std_logic := '0';
    signal coin_pulse500 : std_logic := '0';
    signal coin_pulse1000 : std_logic := '0';
begin
    producto_sel <= to_integer(unsigned(sel_prod));
    ...
    -- Detección de pulso para monedas
    process(clk, reset)
    begin
        if reset = '1' then
            prev_coin500 <= '0';
    ...
    begin
        if reset = '1' then
            ...
            if confirmar = '1' and prev_confirmar = '0' then
                confirm_latched <= '1';  -- Latched hasta que se use
                buzzer_debug <= '1';  -- Activar buzzer debug
                buzzer_debug_cnt <= 50000000;  -- ~1s
            end if;
            prev_confirmar <= confirmar;
            
            -- Reset confirm_latched después de confirmar en WAITING_SELECTION
            if state = WAITING_SELECTION and confirm_latched = '1' then
                confirm_latched <= '0';
            end if;
            
            -- Contador para buzzer debug
            if buzzer_debug_cnt > 0 then
                buzzer_debug_cnt <= buzzer_debug_cnt - 1;
            else
                buzzer_debug <= '0';
            end if;
            
            -- Marcar venta completada cuando entrega termina
            if state = DISPENSING and entrega_done = '1' then
                venta_completada <= '1';
                entrega_activa <= '0';  -- Reset entrega
            else
                venta_completada <= '0';
            end if;
        end if;
    ...
    process(state, sel_prod, coin500, coin1000, confirm_latched, anomalia_sw, stock_actual, saldo_suficiente, entrega_activa)
    begin
        next_state <= state;
        case state is
            when WAITING_SELECTION =>
                if confirm_latched = '1' and stock_actual > 0 and saldo_suficiente = '1' then  -- Condiciones estrictas
                    next_state <= DISPENSING;
                end if;
            when DISPENSING =>
                if entrega_done = '1' then  -- Entrega terminada
    ...
    -- Lógica de salidas
    process(state, clk, reset, stock_actual, cambio, saldo, producto_sel, anomalia_sw, entrega_activa, clk_500ms, clk_2s, venta_completada, confirm_latched, buzzer_debug, coin_pulse500, coin_pulse1000)
    begin
        -- Valores por defecto
        ram_we <= '0';
        ram_addr <= sel_prod;
        ...
        valor_display_full <= 0;  -- Por defecto
        led_compra <= '0';
        stock_display <= stock_actual;  -- Por defecto
        alerta_led <= '0';
        door_led <= '0';
        entrega_led <= '0';
        anomalia_led <= '0';
        buzzer <= buzzer_debug;  -- Incluir debug
        debug_venta <= venta_completada;  -- Debug: alta cuando venta completada
        start_entrega <= '0';  -- Por defecto
        
        if reset = '1' then
            -- Reset global: todo a cero
            reset_stock <= '1';
            ram_saldo_we <= '1';
            ram_saldo_din <= 0;  -- Reset saldo a 0 en RAM
            valor_saldo <= 0;
            valor_producto <= 0;
            valor_display_full <= 0;
        else
            -- Actualizar saldo en RAM al agregar monedas
            if coin_pulse500 = '1' then
                ram_saldo_we <= '1';
                ram_saldo_din <= saldo + 500;
            elsif coin_pulse1000 = '1' then
                ram_saldo_we <= '1';
                ram_saldo_din <= saldo + 1000;
            end if;
            
            case state is
                when WAITING_SELECTION =>
                    -- LEDs de stock
                    stock_display <= stock_actual;
                    -- Alerta si no hay stock
                    if stock_actual = 0 then
                        alerta_led <= clk_2s;
                    end if;
                    if confirm_latched = '1' then
                        led_compra <= '1';
                        -- Restar stock en RAM inmediatamente
                        ram_we <= '1';
                        ram_din <= std_logic_vector(to_unsigned(stock_actual - 1, 2));
                        -- Actualizar stock_display inmediatamente para LEDs
                        stock_display <= stock_actual - 1;
                        -- Mostrar cambio completo en los cuatro displays
                        mostrar_cambio <= '1';
                        valor_display_full <= abs(cambio);  -- Cambio completo para disp0-disp3
                        door_led <= '1';
                        buzzer <= '1';  -- Señal sonora
                    end if;
                when DISPENSING =>
                    -- Iniciar entrega solo aquí
                    start_entrega <= '1';
                    -- Mantener cambio mostrado en los cuatro displays y puerta encendida durante entrega
                    mostrar_cambio <= '1';
                    valor_display_full <= abs(cambio);
                    door_led <= '1';
                    entrega_led <= clk_500ms;  -- Parpadeo durante entrega
                    buzzer <= '1';
                    entrega_activa <= '1';  -- Simular entrega
                    stock_display <= stock_actual - 1;  -- Mantener decrementado
                ...
                when ANOMALY =>
                    ...
                    stock_display <= stock_actual;  -- Mantener
            end case;
        end if;
    end process;
    
    -- Decodificar LEDs de stock basado en stock_display
    process(stock_display)
    begin
        case stock_display is
            when 3 => stock_leds <= "111";
            when 2 => stock_leds <= "110";
            when 1 => stock_leds <= "100";
            when others => stock_leds <= "000";
...
architecture arch of maquina_expendedora is
    ...
    signal d0, d1, d2, d3 : std_logic_vector(3 downto 0);
    signal d0_full, d1_full, d2_full, d3_full : std_logic_vector(3 downto 0);  -- Para cambio completo
    signal disp_d0, disp_d1, disp_d2, disp_d3 : std_logic_vector(3 downto 0);  -- Señales intermedias para mux
    signal clk_1Hz        : std_logic;
    signal clk_500ms      : std_logic;
    signal clk_2s         : std_logic;
...
begin
    ...
    -- BCD para saldo (disp0/disp1) y producto (disp2/disp3) en estados normales
    U_bcd_saldo: bin_bcd
        port map(
            bin => valor_saldo,
            d0  => d0,
    ...
    -- BCD para cambio completo (disp0-disp3) durante compra
    U_bcd_full: bin_bcd
        port map(
            bin => valor_display_full,
            d0  => d0_full,
            d1  => d1_full,
            d2  => d2_full,
            d3  => d3_full
    ...
    -- Mux de displays: proceso para seleccionar entre saldo/prod o full
    process(mostrar_cambio, d0, d1, d2, d3, d0_full, d1_full, d2_full, d3_full)
    begin
        if mostrar_cambio = '1' then
            disp_d0 <= d0_full;
            disp_d1 <= d1_full;
    ...
    U_d0: systemd port map(A => disp_d0, D0 => disp0);
    U_d1: systemd port map(A => disp_d1, D0 => disp1);
    U_d2: systemd port map(A => disp_d2, D0 => disp2);
    U_d3: systemd port map(A => disp_d3, D0 => disp3);
    ...
    U_fsm: fsm_maquina_expendedora
        port map(
            ...
            buzzer        => buzzer,
            debug_venta   => open  -- No conectado, para debug si tienes LED
        );
